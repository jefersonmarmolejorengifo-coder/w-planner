...
**🤝 Consenso de pares**
- **H-053 `[A+B]` MEDIO · Testing — sin pruebas de componente de `AuthScreen`.** Hoy solo helpers + guarda estática (`src/lib/otp.test.js`); autoenvío, candados, enfriamiento y foco se verificaron por lectura y capturas. Añadir jsdom + testing-library. Esfuerzo MEDIO. *Cerrado:* `src/screens/AuthScreen.test.jsx` (jsdom 29 + Testing Library 16, 16 casos incl. CAPTCHA con/sin clave y fallo de Turnstile); 3 sabotajes de producción hacen fallar exactamente la prueba correspondiente.
- **H-054 `[A+B]` MEDIO · Seguridad — controles anti-abuso del código sin reforzar ni declarar.** Sin CAPTCHA en `/otp` con alta abierta (`src/screens/AuthScreen.jsx:64-66`) y tope global de 30 correos/h: cualquiera puede agotarlo y bloquear el login de todos; los límites de `/verify` (30 cada 5 min por IP, documentación oficial) no están declarados en el repo. Turnstile/hCaptcha + declararlos en el script de publicación. Esfuerzo MEDIO. *Cerrado 2026-09-15:* Turnstile activo en producción (widget propio creado por API); un envío sin token se rechaza con `captcha_failed` y el login por código, incluida la renovación de sesión, sigue funcionando; si Turnstile no carga, la pantalla ofrece reintento y contacto. Reversión: `enable-auth-captcha --disable`.
...
**🅰️ Solo Claude (A)**
- **H-056 `[A]` ALTO · Operación — config de Auth de producción desincronizada de la app desplegada, sin detector de deriva.** Dry-run tras el deploy: 13 claves en "CAMBIA". El `--apply` quedó bloqueado por el clasificador de permisos de Claude Code a la espera de autorización de Jefer. Publicar y añadir `--check` (sale ≠0 si hay deriva) en CI o en el cron diario. Esfuerzo BAJO. **Actualización 2026-09-14:** plantillas publicadas y verificadas (14/14 OK) con autorización de Jefer; correo real enviado a la cuenta corporativa sin accesos del escáner. *Cerrado:* `--check` (sale 1 si hay deriva con producción) + `scripts/auth-email/published.json` y un test de huella que el CI corre en cada push (falla si alguien cambia plantillas sin publicarlas).
- **H-062 `[A]` MEDIO · Conexión — el login depende de un tope global de 30 correos/h y el mensaje lo oculta:** `over_email_send_rate_limit` es el mismo código para la espera de 60 s por usuario y para el tope del proyecto, y la UI dice "espera un minuto". Subir el tope según el plan de Resend y matizar el mensaje. Esfuerzo BAJO. *Cerrado 2026-09-15:* mensaje corregido y tope en 100/h, activado en el mismo paso que el CAPTCHA (subirlo antes solo habría dado más margen a un abuso).
...
- **H-065 `[A]` BAJO · UX — un enlace viejo que ya no sirve devuelve al login sin explicación** (`src/ProductivityPlus.jsx:501-512` no lee el `error_code` de la URL). *Cerrado:* `src/lib/initialAuthUrlError.js` captura el error antes de crear el cliente de Supabase, limpia solo ese hash (conserva `?join=`, no toca `access_token`) y la pantalla muestra el aviso; verificado en navegador.
...
- **H-058 `[B]` MEDIO · Conexión — el script de publicación usa el ref de producción por defecto.** Mitigado: imprime el nombre del proyecto y aborta si el ref no responde. *Cerrado:* `--apply` exige `SUPABASE_PROJECT_REF` explícito y aborta antes de cualquier llamada si falta.
