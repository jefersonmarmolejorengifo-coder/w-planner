...
| **H-022** metadata de modelo | ✅ servidor listo · ⚠️ UI pendiente de decisión | Nueva fuente única `src/aiModels.js`. Corregido el header `X-Wplanner-Model` del semanal (decía Opus, llama Sonnet) y centralizados todos los endpoints. **Los rótulos de marketing en la UI (REPORT_TYPES dicen "Opus 4.7" para semanal/mensual que en realidad usan Sonnet 4.6) NO se cambiaron — es decisión de producto.** |
...
| **H-020** RPCs con anon | ✅ código listo · ⏳ migración pendiente | `migrations/031`: revoca `anon` de `project_has_feature`/`can_use_evolutivo`/`can_use_chat`/`chat_quota_remaining`/`user_can_use_ia_on_project`. Sin guard de membresía (cron las llama con service_role). |

**37 tests** en verde (+8: sanitización y consistencia de modelos). Pendiente operativo: aplicar **migración 031**.
